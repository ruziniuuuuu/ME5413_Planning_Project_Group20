...
double PathTrackerNode::computeStanleyControl(const double heading_error, const double cross_track_error, const double velocity)
{
  ...
  return std::min(std::max(stanley_output, -2.2), 2.2);
    ...
    void PathTrackerNode::adjustStanleyKBasedOnVelocity(double velocity) {
        const double LOW_SPEED_THRESHOLD = 0.5; // 低速阈值，单位：米/秒
        const double HIGH_SPEED_THRESHOLD = 2.0; // 高速阈值，单位：米/秒
        const double STANLEY_K_MIN = 0.5;
        const double STANLEY_K_MAX = 2.0;
        const double smoothFactor = 0.1; // 平滑因子
        ...
        // 使用平滑因子更新STANLEY_K
        STANLEY_K = smoothFactor * newStanleyK + (1 - smoothFactor) * prev_STANLEY_K;
        prev_STANLEY_K = STANLEY_K; // 更新上一个STANLEY_K值以用于下次调整
    }
