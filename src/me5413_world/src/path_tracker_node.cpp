...
// Method
int METHOD = 0; // 0 for Stanley (given as default), 1 for Pure Pursuit, 2 for LQR, and 3 for MPC
...
double PathTrackerNode::computePurePursuitControl(const double heading_error, const double cross_track_error, const double velocity)
{
  const double stanley_output = -1.0*(heading_error + std::atan2(STANLEY_K*cross_track_error, std::max(velocity, 0.3)));
...
double PathTrackerNode::computeLQRControl(const double heading_error, const double cross_track_error, const double velocity)
{
  // LQR Control
  const double A = 0.5;
  const double B = 0.5;
  ...
  const double lqr_output = -1.0*(A*heading_error + B*cross_track_error + C*velocity + D*velocity);

  return std::min(std::max(lqr_output, -2.2), 2.2);
};

double PathTrackerNode::computeMPCControl(const double heading_error, const double cross_track_error, const double velocity)
{
  // MPC Control
  const double mpc_output = -1.0*(heading_error + cross_track_error + velocity);

  return std::min(std::max(mpc_output, -2.2), 2.2);
};
...
geometry_msgs::Twist PathTrackerNode::computeControlOutputs(const nav_msgs::Odometry& odom_robot, const geometry_msgs::Pose& pose_goal)
{
  ...
  if (METHOD == 1) {
    ...
    cmd_vel.angular.z = computePurePursuitControl(heading_error, lat_error, velocity);
  } else
  if (METHOD == 2) {
    cmd_vel.linear.x = SPEED_TARGET;
    cmd_vel.angular.z = computeLQRControl(heading_error, lat_error, velocity);
  } else
  if (METHOD == 3) {
    cmd_vel.linear.x = SPEED_TARGET;
    cmd_vel.angular.z = computeMPCControl(heading_error, lat_error, velocity);
  } else {
    // Wrong Method, then stop
